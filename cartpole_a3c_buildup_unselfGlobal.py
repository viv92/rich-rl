...
class Worker(mp.Process):
    ...
    def run(self):
        ...
        while episode_counter.value < max_episodes:
            ...
            # eps decay
            if episode_counter.value % (max_episodes//10) == 0:
                self.eps -= (1/7)
            ...
            while not done:
                ...
                if done or (ep_steps % update_steps == 0):
                    ...
                    with self.optimization_lock:
                        ...
                        # g_optimizer_v.zero_grad()
                        # g_optimizer_p.zero_grad()

                        # copy lnet grads to gnet grads
                        for lp, gp in zip(self.lnet_p.parameters(), gnet_p.parameters()):
                            gp._grad = lp.grad.clone()
                        for lp, gp in zip(self.lnet_v.parameters(), gnet_v.parameters()):
                            gp._grad = lp.grad.clone()
                        ...
                        # sync lnet params with gnet params
                        self.lnet_v.load_state_dict(gnet_v.state_dict())
                        self.lnet_p.load_state_dict(gnet_p.state_dict())
            ...
            # store ep stats
            with self.optimization_lock:
                ep_return_list.put(ep_return)

            if episode_counter.value % (max_episodes//10) == 0:
                print('ep:{} \t ep_return:{} \t worker:{}'.format(episode_counter.value, ep_return, self.name))
